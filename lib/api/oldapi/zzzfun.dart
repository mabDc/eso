...
class ZZZFun implements API {
  ...
  Future<List<SearchItem>> search(String query, int page, int pageSize) async {
    final res =
        await http.get('http://www.zzzfun.com/vod-search-page-$page-wd-$query.html');
    return parse(res.body).querySelectorAll('#list-focus li').map((li) {
      ...
      li.querySelectorAll('dl').forEach((dl) {
        if (dl.querySelector('dt') != null) {
          info.addAll({
            dl.querySelector('dt').text.replaceAll('：', ''): dl.querySelector('dd').text
          });
        }
      });
      return SearchItem(
  ...
  Future<List<String>> content(String url) async {
    ...
    RegExpMatch regExpMatch =
        RegExp('/vod-play-id-(?<vid>\\d+)-sid-(?<sid>\\d+)-nid-(?<nid>\\d+)\\.html')
            .firstMatch(url);
